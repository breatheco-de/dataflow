...
def pull_project_from_github(project):
    ...
    for pipeline in config['pipelines']:
        ...
        if other_pipeline_using_same_destination is not None:
                raise Exception(
                    f"Another pipeline {other_pipeline_using_same_destination.slug} ({str(other_pipeline_using_same_destination.id)}) is already using destination datasource {destination.slug}, this pipeline is {pipelineObject.slug} ({str(pipelineObject.id)})"
                )
