    ...
    def get_config(self, pipeline_slug=None):
        ...
        for pipeline in self.config["pipelines"]:
            if not isinstance(pipeline, dict):
                raise Exception(
                    "Project pipelines property must be a dictionary on the YML"
                )
            ...
            if "transformations" not in pipeline:
                raise Exception(
                    f'Pipline {pipeline["name"]} is missing transformations list on the YML'
                )
            if not isinstance(pipeline["transformations"], list):
                raise Exception(
                    "Project.pipelines[].transformations property must be a list on the YML"
                )

            count = len(self.config["pipelines"][pipeline_index]["transformations"])
            for index in range(count):
                self.config["pipelines"][pipeline_index]["transformations"][index] = (
                    self.config["pipelines"][pipeline_index]["transformations"][
                        index
                    ].split(".")[0]
                )
...
class DataSource(models.Model):
    ...
    table_name = models.TextField(
        ...
        blank=True,
        default=None,
        help_text="Ignored for CSV. You can write a SELECT query statement, but for Heroku, SQL or Big Query only.",
    )
...
class Pipeline(models.Model):
    ...
    source_from = models.ManyToManyField(
        DataSource, blank=True, related_name="pipeline_from_set"
    )
    ...
    def destination_table_name(self):
        ...
        if (
            self.source_to.source_type == "csv"
            and self.source_to.connection_string is not None
        ):
...
class PipelineExecution(models.Model):
    ...
    incoming_stream = models.JSONField(
        blank=True,
        null=True,
        default=None,
    ...
    def buffer_url(self, position=0):
        return (
            "./buffer/" + str(self.id) + self.pipeline.slug + f"_buffer{position}.csv"
        )
